...
Note: We won't be doing *all* of these examples separately in the workshop, but additional examples are here for you to explore. 
...
b. Now let's made an animated version using transition_states:
...
e. Now that the static version is working, make an animated version using transition_manual
...
f. Use transition_layers to animate geoms on top of each other
...
g. An transition_layers example with previous layers NOT retained (and exit_ updated) - here, height by planet (Tatooine or Naboo) ignoring all other variables
...
###3. Abalone + ridge plots + transition_states (Joy Division meets action figures)
...
ggplot(abalone, aes(x = length_mm, y = age_years, fill = sex, color = sex)) +
  geom_density_ridges(alpha = 0.5) +
  scale_fill_manual(values = c("purple","navy","aquamarine")) +
  scale_color_manual(values = c("purple","navy","aquamarine")) + theme_minimal() +
